	...
	if (event.pressure > 0.1) {
		if (!slider.get("wasMoved")) {
			slider.set(
				"wasMoved",
				!!(
					event.x > slider.get("pointerPos") + 2 ||
					event.x < slider.get("pointerPos") - 2
				),
			);
			return;
		...
		if (!removeActiveFromBilder()) {
			const newPos =
				slider.get("scrollPos") +
				(slider.get("pointerPos") - event.x) *
...
bilderRow.addEventListener("keydown", (keyEvent) => {
	...
	if (
		...
		if (bilderRow.querySelector(".Bild.focus")) {
			newFocusIndex = BilderArr.findIndex((el) =>
...
function setBilderScrollPos(value) {
	...
	if (slider.get("scrollPos") > slider.get("maxPos"))
		slider.set("scrollPos", slider.get("maxPos"));
...
function getOffsetForElementCentering(targetEl) {
	let pictureRect;
	if (
		targetEl.tagName === "PICTURE" &&
		targetEl.classList.contains("BildCont")
	) {
		...
		if (closestBildCont) {
			...
			console.error(
				"no picture.BildCont element found at getOffsetForElementCentering()",
			);
		}
	}
	const offset =
		slider.get("scrollPos") +
		(pictureRect.x - (window.innerWidth - pictureRect.width) / 2);
	return offset;
}
...
function switchImgResolution(el) {
	...
	function getNewSize(w, h) {
		...
		if (imgAspectRatio >= 1) {
			return (
				bilderSizes.find((size) => size >= w * screenPixelRatio) ||
				bilderSizes[bilderSizes.length - 1]
			);
		}
		// hochformat:
		if (imgAspectRatio < 1) {
			return (
				bilderSizes.find((size) => size >= h * screenPixelRatio) ||
				bilderSizes[bilderSizes.length - 1]
			);
		}
	}
	...
	if (!el.classList.contains("active")) {
		if (windowAspectRatio > imgAspectRatio) {
			...
			for (const sourceEl of sourceEls) {
				sourceEl.srcset = sourceEl.srcset.replace(
					/\d+\./,
...
function removeActiveFromBilder() {
	...
	if (activeImgs.length > 0) {
		...
		setBilderScrollPos(
			getOffsetForElementCentering(activeImgs[activeImgs.length - 1]),
		);
...
function addOnlyfansBtn() {
	...
	if (headerSocialCont) {
		...
		console.warn(
			'could not append Onlyfans Button. ".headerSocialCont" not found.',
		);
	}
}
		...
		(entries) => {
			for (const entry of entries) {
				entry.target.style.setProperty(
					"--sectionHeadlineBrightness",
					Number.parseFloat(entry.intersectionRatio * 0.13 + 0.45).toFixed(2),
				);
				entry.target.style.fontWeight = Number.parseInt(
					entry.intersectionRatio * 150 + 100,
				);
			}
		},
		...
		const illustrationBgBlurObserver = new IntersectionObserver(
			(entries) => {
				for (const entry of entries) {
					...
					bgEl.style.scale = Number.parseFloat(
						entry.intersectionRatio * 0.1 + 1.0,
					).toFixed(3);
				}
			},
			{
